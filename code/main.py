...
def getArgs():
    ...
    parser.add_argument("-ei", help = "input the end index", dest = "endindex", type=int, default=4001)
    parser.add_argument("-c", help = "input the configuration file id", dest = "id", type=str, default="0")
    parser.add_argument("-e", help = "input the name of an excel file in the dataset directory", dest = "excel_name", type = str, default = "large_scale.xlsx")
    parser.add_argument("-l", help = "input the path to save logs", dest = "log_path", type = str, default = "../../output/large_scale/")
    parser.add_argument("-m", help = "input the LLM", dest = "llm", type = str, default = "DeepSeek")
    args = parser.parse_args()
    INDEX = args.index
    END_INDEX = args.endindex
    CONFIG_ID = args.id
    EXCEL_PATH = os.path.join(os.path.abspath(r".."), "dataset", args.excel_name)
    LOG_PATH = args.log_path
    LLM = args.llm
    return INDEX, END_INDEX, CONFIG_ID, EXCEL_PATH, LOG_PATH, LLM
...
if __name__ == '__main__':
    INDEX, END_INDEX, CONFIG_ID, EXCEL_PATH, LOG_PATH, LLM = getArgs()
